...
如果其他取碼網站把密鑰放在網址路徑或查詢參數裡，例如 `/2fa/密鑰` 或 `?secret=密鑰`，直接開啟連結時，瀏覽器會把密鑰隨 HTTP 請求傳送出去，對方的遠端網站服務或 CDN 能收到這份資料，也可能透過存取日誌、監控或應用程式記錄並留存。是否實際記錄、保留多久，取決於對方的設定與政策。
...
- 連結直達：開啟 [https://2fa.hot/2fa#你的密鑰](https://2fa.hot/2fa#你的密鑰)，直接顯示目前驗證碼。密鑰放在 `#` 後，由瀏覽器本地讀取與計算，不隨 HTTP 頁面請求傳送至遠端。
...
新直達連結使用 `/2fa#密鑰`，密鑰和驗證參數位於 # 後面，由瀏覽器本地讀取，不隨頁面請求傳送至託管服務。舊 `/2fa/密鑰` 連結仍可使用，但網址路徑中的密鑰會隨請求送達託管服務。完整連結仍含密鑰，也可能留在瀏覽器歷史裡，請勿公開分享或交給不信任的人。忘記本機紀錄口令後，網站無法幫你找回。
