...
它不僅支援自動識別/切換批量數據，同時支援多種密鑰格式 - 無論是從Excel表單複製過來的傻逼格式、或因您的客戶不會使用、連帶密碼+密鑰一起複製過來，都能識別並有相應提示。
...
- 連結直達：支援開啟 [https://2fa.hot/2fa#你的密鑰](https://2fa.hot/2fa#你的密鑰)，直接顯示目前驗證碼。密鑰放在 `#` 後，由瀏覽器本地讀取與計算，不隨 HTTP 頁面請求傳送至遠端，避免密鑰隨這次請求傳出。
- 本機紀錄：預設不儲存記錄，可主動開啟；密碼保護可隨時開啟或取消，支援備份與還原。
- 30 種語言：適配手機、深淺主題；不會用就點教學，跟著鑽石劍走一遍。

支援 TOTP、SHA-1／SHA-256／SHA-512、6／8 位驗證碼。HOTP、Steam 專用格式不參與取碼；遷移檔案中的 HOTP／MD5 帳號可解析匯出。

Google Authenticator 匯入：在 Google驗證器 App 中選擇「轉移帳號 → 匯出帳號」，再到本站「匯入 QR 碼 → Google Authenticator」掃描/上傳，即可導出密鑰至任何地方。

…And More

## 怎麼用

1. 複製 2FA 密鑰，在 2fa.hot 貼上密鑰或匯入 QR 碼。
2. 在倒計時内複製目前驗證碼，填到要驗證的網站或 App。快過期時等下一組。
3. 生成全由本地處理的直接連結，一鍵複製/批量生成。

## 密鑰放在哪裡

首頁取碼和 QR 碼辨識 完全在瀏覽器內完成。

主動開啟本機紀錄後，紀錄會儲存在你的瀏覽器中。驗證碼計算、QR 碼辨識與歷史紀錄儲存均在本地完成。還可以設定密碼加密保護，也可以不設密碼，直接查看。

新直達連結使用 `/2fa#密鑰`，密鑰和驗證參數位於 `#` 後面，僅由瀏覽器本地讀取，不會隨頁面請求傳至託管服務。舊 `/2fa/密鑰` 連結仍可兼容使用，但網址路徑中的密鑰會隨請求送達託管服務。專案部署設定已停用 Workers 日誌與 Logpush；這不會改變舊連結傳送密鑰的方式。完整連結仍含密鑰，也可能留在瀏覽器歷史裡，所以我們不推薦您使用舊路徑式直鏈。
...
...假如我寫這段話的時候繃住了呢。於是.HOT 來了。好吧，好吧，我承認這是性感的，我們所以回去。

## 為什麼做這個？重複造輪子？

在過去，我用過很多類似的在綫工具，直鏈取得代碼。如果你不知道的話，只要訪問帶密鑰的網址，例如 `/2fa/密鑰` 或 `?secret=密鑰`，瀏覽器會把密鑰隨 HTTP 請求傳送出去，對方的遠端網站服務或 CDN 能收到這份資料，也能透過存取日誌、監控或應用程式記錄並留存。也就是說，這些請求中的密鑰可能被明文儲存；假如你曾經還貼入過賬戶+密碼+2FA，而對方也上傳並留存這些內容... BOOM! 當然，是否實際留存取決於對方的實作、設定與隱私政策:))

...大概是2021年，第四次工業革命，人類進入AI時代。由此，經常看到有人在濫用，甚至專業程序員，也乾這種勾當——只用5分鐘 就生成一個他媽的漸變、不對齊、用了100個不同設計風格ui、1000種元素的前端。

像是某些連我訊息都不敢回的人，只用AI隨便做一個粗製濫造的垃圾系統 就能賣貨給灰產用戶發大財。

…這一切看得我噁心，我從床上爬起來，吐了一遍又一遍，可惜沒被穩穩接住。

我想要做的 有钱人都做过了。2fa.hot 幾乎由 Vibe Coding 完成——當然，我將它改到勉强滿意才上綫。正如我和同事的某些項目一樣，我們希望早日上綫，但止不住一遍遍修訂，直到符合我們的要求，直到它從未上綫。某些項目也許未來會被讀這條文字的各位熟知，儘管你們也不會知道那是我們做的。這感覺 像是回到6年前，打開Premiere Pro，全身心投入創作，樂此不疲。可如今我卻再做不出影片了，令人感嘆。

我想要说的 前人们都说过了。我不反對AI。6年前 我是文科生，我可能連他媽怎麽開個網站都不知道。我希望 某些人能善用AI。它能幫你做事，但不能成爲你，永遠。請別用它代替你的想法，代替你的人生；別用它寫出個自己都懶得玩的設計、各種影響體驗的BUG邏輯… 快用localhost打開，繼續打磨100遍。否則，那不是你自己的，別放出來惡心任何人。賣錢？你不配，只能一直在灰色產業摸爬滾打。

我想要的公平都是不公们虚构的。我知道 我改變不了當今快節奏的生活，也攔不住的那些。它們。
