...
多功能線上 2FA 工具, 以 Minecraft 風格為設計參考。貼上2FA密鑰，取得目前驗證碼。支援直鏈，取碼全由本地處理。
...
由於好友做跨境電，例如媒體賬戶運營需的登錄需求，還需考慮安全性。想到能爲更多此類朋友們、乃至與剛接觸互聯網的朋友提供幫助、以此為出發點。
網路上有很多類似的在綫工具，還能以直鏈取得代碼。但如果你不知道的話，只要訪問直接帶密鑰的網址，例如 `/2fa/密鑰` 或 `?secret=密鑰`，瀏覽器會把密鑰隨 HTTP 請求傳送出去，對方的遠端網站服務或 CDN 能收到這份資料，也能透過存取日誌、監控或應用程式記錄並留存。也就是說，這些請求中的密鑰可能被明文儲存；假如你曾經還貼入過賬戶+密碼+2FA，而對方也上傳並留存這些內容... BOOM! 當然，是否實際留存取決於對方的實作、設定與隱私政策:))
