...
如果其他取码网站把密钥放在网址路径或查询参数里，例如 `/2fa/密钥` 或 `?secret=密钥`，直接打开链接时，浏览器会把密钥随 HTTP 请求发送出去，对方的远端网站服务或 CDN 能收到这份数据，也可能通过访问日志、监控或应用程序记录并留存。是否实际记录、保留多久，取决于对方的设置与政策。
...
- 连结直达：开启 [https://2fa.hot/2fa#你的密钥](https://2fa.hot/2fa#你的密钥)，直接显示目前验证码。密钥放在 `#` 后，由浏览器本地读取与计算，不随 HTTP 页面请求发送至远端。
...
新直达链接使用 `/2fa#密钥`，密钥和验证参数位于 # 后面，由浏览器本地读取，不随页面请求发送至托管服务。旧 `/2fa/密钥` 链接仍可使用，但网址路径中的密钥会随请求送达托管服务。完整链接仍含密钥，也可能留在浏览器历史里，请勿公开分享或交给不信任的人。忘记本地记录口令后，网站无法帮你找回。
