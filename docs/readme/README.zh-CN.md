...
支持 TOTP（SHA-1／SHA-256／SHA-512、6／8 位）和 Steam Guard（5 位、30 秒）。Steam 可输入 shared_secret 或粘贴 maFile JSON 内容。HOTP 不参与取码；迁移档案中的 HOTP／MD5 帐号可解析导出。

## 怎么用

- 复制 2FA 密钥，在 2fa.hot 贴上密钥或汇入 QR 码。
- 在倒计时内复制使用目前验证码。
